...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const existingCount = await prisma.fixname.count();
    if (existingCount > 0) {
    ...
    // Read CSV file
    const csvPath = path.join(process.cwd(), 'src', 'app', 'csv', 'fixname.csv');
    ...
    // Find column indices
    const shortlistIndex = header.findIndex(h => h.trim().toLowerCase() === 'shortlist');
    ...
    if (shortlistIndex === -1 || listIndex === -1 || companyIndex === -1) {
      return Response.json(
        { error: 'Invalid CSV format: missing shortlist, list or company column' },
        { status: 400 }
      );
    }
    ...
    const dataRows = lines.slice(1).map(line => {
      ...
      return {
        shortlist: values[shortlistIndex] || '',
    ...
    // Insert data in bulk
    const result = await prisma.fixname.createMany({
      data: dataRows,
      skipDuplicates: true
    });
...
export async function GET() {
  ...
  try {
    const count = await prisma.fixname.count();
    return Response.json({
      imported: count > 0,
      count
    });
