  ...
  try {
    ...
    if (!company) {
      return Response.json(
        { error: 'Company is required', imported: false },
        { status: 400 }
      );
    }

    // Check if data for this company already imported (prevent duplicate import)
    ...
    // Read CSV file
    const csvPath = path.join(process.cwd(), 'src', 'app', 'csv', 'Labeldata_PD.csv');
    ...
    const dataRows = lines.slice(1).map(line => {
      ...
      return {
        company: company, // Use company from request body
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!company) {
      return Response.json({ imported: false, count: 0 });
    }
