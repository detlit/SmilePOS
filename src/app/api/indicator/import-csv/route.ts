...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const existingCount = await prisma.indicator.count({
      where: { company: 'A' }
    });
    if (existingCount > 0) {
    ...
    // Read CSV file
    const csvPath = path.join(process.cwd(), 'src', 'app', 'csv', 'Indicatoredit.csv');
    ...
    // Insert data in bulk
    const result = await prisma.indicator.createMany({
      data: dataRows,
      skipDuplicates: true
    });
