...
const BodyCat = () => {
  ...
  const [shortlist, setlistS] = useState('')
  ...
  const checkImportStatus = async () => {
    try {
      const res = await axios.get('/api/fixname/import-csv')
  ...
  const handleImportCSV = async () => {
    ...
    setIsImporting(true)
    try {
      const res = await axios.post('/api/fixname/import-csv')
      if (res.data.success) {
  ...
  return (
    <>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', padding: '10px' }}>
          ...
          <div className={styles.productInfoCardBody}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, padding: '10px', background: '#f8f9fa', borderRadius: 8 }}>
              <input
                ...
                placeholder="ชื่อทางการ / Generic Name"
        ...
        {/* Right Column - Sample Fixnames Card */}
        <div className={styles.pricingCard} style={{ background: 'linear-gradient(135deg, #fff 0%, #fef3c7 100%)' }}>
          <div className={styles.pricingCardHeader} style={{ background: 'linear-gradient(135deg, #E5EEF8 0%, #CCDFF1 100%)', color: '#1E5088', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div><span>📋</span> ตัวอย่าง ชื่อทางการ</div>
            <button
          ...
          <div className={styles.pricingCardBody} style={{ padding: 0, maxHeight: '70vh', overflowY: 'auto' }}>
            <Table className="table" size="sm">
              <thead style={{ position: 'sticky', top: 0, background: '#f3f4f6' }}>
                <tr>
                  <th className={styles.bodydetailTable_Re} style={{ width: '80%', color: '#374151', fontSize: 10, fontWeight: 'bold' }}>ชื่อทางการ</th>
