...
const BodyCat = () => {
  ...
  const checkImportStatus = async () => {
    try {
      const res = await axios.get('/api/getagory/import-csv')
  ...
  const handleImportCSV = async () => {
    ...
    setIsImporting(true)
    try {
      const res = await axios.post('/api/getagory/import-csv')
      if (res.data.success) {
  ...
  return (
    <>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', padding: '10px' }}>
        ...
        {/* Left Column - Category Management Card */}
        ...
        {/* Right Column - Sample Categories Card */}
        <div className={styles.pricingCard} style={{ background: 'linear-gradient(135deg, #fff 0%, #F3F8FC 100%)' }}>
          <div className={styles.pricingCardHeader} style={{ background: 'linear-gradient(135deg, #E5EEF8 0%, #CCDFF1 100%)', color: '#173F6B', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div><span>📋</span> ตัวอย่าง หมวดสินค้า</div>
            <button
          ...
          <div className={styles.pricingCardBody} style={{ padding: 0, maxHeight: '70vh', overflowY: 'auto' }}>
            <Table className="table" size="sm">
              <thead style={{ position: 'sticky', top: 0, background: '#f3f4f6' }}>
                <tr>
                  <th className={styles.bodydetailTable_Re} style={{ width: '80%', color: '#374151', fontSize: 10, fontWeight: 'bold' }}>หมวดสินค้า</th>
